<template>
    <label :for="['switch-', props.id]" :aria-disabled="props.disabled"
        :class="[props.className, 'relative inline-block h-8 w-14 cursor-pointer rounded-full bg-gray-300 transition [-webkit-tap-highlight-color:_transparent] has-[:checked]:bg-pink-500']">
        <input type="checkbox" :disabled="props.disabled" @click="onClick" :id="['switch-', props.id]"
...
const props = withDefaults(defineProps<SwitchInterface>(), {
    checked: false,
    disabled: false
})

const emmit = defineEmits(['onClick'])
